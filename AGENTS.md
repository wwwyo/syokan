...
- **Reachability decides whether a server exists, not the pidfile.** The CLI probes `/api/health` (which reports `version` + `pid`) and adopts whatever answers, rewriting the pidfile from the live `pid`; the pidfile is a cache consulted only for pre-`pid` builds. Judging by "is the recorded pid alive?" broke as soon as the file went stale: the CLI spawned a rival that could never bind the occupied port, timed out after 15s on *every* call, and left the failed child running — an agent looping over the CLI piles up servers that way. Hence also: a readiness timeout kills the child it spawned, and a port answering as non-syokan is reported as a port conflict rather than a timeout (that distinction is what `GET /api/catalog`'s shape is probed for once health fails).
