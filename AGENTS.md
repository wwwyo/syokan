...
- **Backend**: `Bun.serve({ routes })` — the frontend is served via the `index.html` import (dev: on-the-fly bundle, rebundled per reload with HMR off — see server/index.ts; compile: embedded in the binary). `/api/*` cohabits in the same process
- **Validation**: Zod
...
- **The frontend is a static `import index from "../index.html"`**. Dev gets an on-the-fly bundle (rebundled per reload; HMR is deliberately off, see the comment in server/index.ts); at compile time Bun bundles the frontend and embeds it in the binary (the same static import serves both).
