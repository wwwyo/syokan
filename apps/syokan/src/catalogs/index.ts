...
const entries: readonly ViewComponentEntry[] = [
  ...
  defineViewComponent("Graph", graphPropsSchema, Graph, {
    childrenTypes: [],
    notes:
      "Interactive architecture diagram (React Flow + dagre, pan/zoom, auto-laid-out). Color is minimal: hotspot (amber) is the only colored role, meaning \"has findings\"; changed vs. neutral is contrast, not hue (changed=normal text, neutral=muted); added/removed carry a `+ `/`− ` label prefix (removed is also struck through and dashed) instead of color, all applied by the renderer — never describe colors in prose. Any node with `href` gets a trailing `↗` in its label so \"click to jump\" is visible in the figure itself. Use for an overview of a changed area: `groups` draw module/file boundaries (nodes reference one via `group`), `sub` is a one-line \"what changed\" per node, and `href:\"#<id>\"` on a node jumps to any node carrying that `id` (typically the finding's `Heading`). `direction:\"LR\"` widens instead of growing tall. node/group ids must be unique and every edge from/to or node group must name one that exists; both are rejected at ingest. Renders a legend of the roles in use, so producers need not explain colors.",
  }),
