...
name: syokan
license: MIT
description: "Build and POST a JSON snapshot envelope to syokan (召喚 — a verb: summon data into a rich, living view). Use when the user says \"syokan this\", \"syokan X\", \"show in syokan\", \"post a snapshot\", \"preview this markdown\", or in Japanese 『〜を syokan』『syokan に出して/表示して/投げて』『snapshot を作って/送って』『syokan のUIで見たい』『Markdown をプレビューして/md ファイルをブラウザで見たい』 — for RSS feeds, in-progress PR reviews, review risk panels, meeting notes, today's TODO, dashboards, or any aggregated data the user wants to see as structured UI. Compose the tree only from catalog components (Stack, Card, Heading, Link, Text, Time, PlainText, Diff, Code, Badge, Mermaid, TreeDoc, Table, Stat, Checklist, Collapsible, TagFilter, Graph, Probe, Markdown) and send it via the syokan CLI or POST /api/snapshots. There is no free-form markdown ingest — the `Markdown` node renders prose flow only (paragraphs, plain lists, emphasis, links, blockquotes, fenced code) and rejects headings/tables/task-lists/raw HTML/images at POST time; those still go through their own catalog node. Never write JSX. Whenever the word syokan appears, use this skill even if the user does not explicitly say snapshot."
---
