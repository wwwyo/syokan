...
| Architecture overview of the changed area | changes span 2+ modules, or the reader has not touched this code recently | a level-3 `Heading`, then a `Text` saying what the figure shows and how to read it (boxes, boundaries, arrow direction, what each color means here), then a `Text` with what to take from it (two sentences), then the `Graph`: node `label` = path relative to the group ("Graph/layout.ts"), `sub` = what the file does and what changed in it ("node placement; replaced by dagre"), `groups` for directories, `role: changed` for touched-but-clean, `hotspot` + `href:"#<finding id>"` where findings are, edge `label` = the relation ("import", "registers"), `direction:"LR"` when wide | a node whose label is a bare file name ("index.ts") with no role in `sub` tells the reader nothing; caption states the conclusion, not a label |
