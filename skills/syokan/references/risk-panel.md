...
| Background | always; depth set by the reader | `Heading` "Background: …" + two short `Text`s (behavior before the change; why the change became necessary) + a `Table` term / meaning | concise; no before / after table (What); Issue, not Background, holds the detailed obstacles |
