...
**Self-similar at every depth.** The panel has the same shape at every zoom level, so the reader can stop at any depth and still hold a complete, consistent picture. The title is the whole panel in one line: `<subject> — <what changed> · <verdict>`. Each section heading is `<PR section> — <what this section is about, in the reader's words>: <its conclusion>` ("Background — why this change: the old Graph could not draw an overview"; review-layer sections use `Review —`), never a label alone ("Background") and never a conclusion alone ("The old Graph could not draw an overview") — the reader must know what question the sentence answers before they can use it. The PR section names (What / Background / Testing / Summary) are the reader's own vocabulary from writing PRs, which is why they may appear where node names may not. Each finding repeats the panel's own order — what and where → why it matters → evidence → what to decide — and its heading is that finding in one line (`<severity> — <what> in <where>`). A cockpit `Table` row is the finding's heading split into cells. If a heading could be moved to a different body without becoming wrong, it is a label; rewrite it as the conclusion.
