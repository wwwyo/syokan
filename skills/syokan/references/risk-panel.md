...
| Architecture overview of the changed area | changes span 2+ modules, or the reader has not touched this code recently | a level-3 `Heading`, then a `Text` saying what the figure shows and how to read it (boxes, boundaries, arrow direction, that hotspot boxes jump to findings, that it pans); colors need no explaining because the node renders a legend of the roles in use; then a `Text` with what to take from it (two sentences), then the `Graph`: node `label` = path relative to the group ("Graph/layout.ts"), `sub` = the file's role in a few words ("node placement"; the change is carried by the role color, not the text), `groups` for directories, `role: changed` for touched-but-clean, `hotspot` + `href:"#<finding id>"` where findings are, edge `label` = the relation ("import", "registers"), `direction:"TB"` first when there are three or more groups (TB puts groups side by side and usually fits the column; LR stacks them and grows wide), `"LR"` for a single chain of few nodes | a node whose label is a bare file name ("index.ts") with no role in `sub` tells the reader nothing; a `sub` longer than a few words is clipped to one line; caption states the conclusion, not a label |
