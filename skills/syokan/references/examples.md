...
Graph pairs (`role`: `added` / `removed` / `changed` / `neutral`; colors fixed by the renderer and meaning the change kind only) go in a horizontal `Stack` for before/after dependency contrasts — see `syokan catalog` for props. A grouped overview with `href` jumps into findings (`groups` for module boundaries, `role: changed` + `href:"#<id>"` where findings concentrate) is the recommended first section of a review panel — see [risk-panel.md](risk-panel.md).
