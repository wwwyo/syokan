...
export const settingsPatchSchema = z.object(settingsShape).partial().strict();
export type SettingsPatch = z.infer<typeof settingsPatchSchema>;

// 永続 file の読み出し用。手置き / 旧 version で欠損・未知キーが混じりうるので、strict に
// せず未知キーは drop し既知キーだけ拾う (欠損は呼び出し側が DEFAULT_SETTINGS で補う)。
export const storedSettingsSchema = z.object(settingsShape).partial();
