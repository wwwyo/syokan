...
`idempotencyKey` 無しの POST は常に新規作成(`201`)。`idempotencyKey` ありは、一致すれば `200`(その場で更新)、一致が無ければ `allowMissing` 指定時は `201`、それ以外は `404`(`not_found`)。検証エラーは `400`（`invalid_json` / `validation_failed`）。CLI コマンドは `syokan --help`。
